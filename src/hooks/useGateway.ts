...
export function useGateway() {
  ...
  const handleChatEvent = useCallback((event: ChatEvent) => {
    if (event.state === 'delta' && event.message?.content) {
      streamBufferRef.current += extractText(event.message.content);
      ...
      const finalContent = (event.message?.content ? extractText(event.message.content) : '') || streamBufferRef.current;
      setMessages(prev => {
        const last = prev[prev.length - 1];
        if (last?.streaming) {
          return [
  ...
  const handleMessage = useCallback((data: string) => {
    try {
      ...
      if (frame.type === 'res' && frame.ok && frame.id?.startsWith('history-')) {
        const history = frame.payload as { messages?: Array<{ role: string; content: unknown; timestamp?: number }> };
  ...
  const connect = useCallback((gatewayUrl: string, token: string) => {
    ...
    ws.onmessage = (e) => {
      ...
      try {
        ...
        if (!connectHandled && frame.type === 'res' && frame.id === connectReqId) {
          connectHandled = true;
          if (frame.ok) {
