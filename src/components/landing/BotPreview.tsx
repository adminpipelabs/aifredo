...
const conversation = [
  ...
  { role: 'bot' as const, text: "Done! I pulled their last 3 emails, the open proposal, and their payment history. Notes are in your Obsidian vault under /clients/acme.", delay: 2800 },
...
export function BotPreview() {
  ...
  return (
    <div className="relative w-full max-w-[340px]">
      ...
      <div className="absolute -inset-4 bg-blue-100/40 rounded-[3rem] blur-2xl" />

      {/* Phone frame — kept dark for contrast */}
      <div className="relative bg-slate-950 rounded-[2.5rem] border border-slate-800 shadow-2xl overflow-hidden">
        {/* Notch */}
        ...
        <div className="px-3 py-4 space-y-3 h-[360px] overflow-hidden">
          ...
          {visibleMessages < conversation.length && visibleMessages > 0 && conversation[visibleMessages].role === 'bot' && (
            <div className="flex justify-start">
              <div className="px-4 py-3 rounded-2xl bg-slate-800 border border-slate-700/30 rounded-bl-md">
      ...
      <div className="absolute -right-3 top-20 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-lg">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-[11px] text-slate-700 font-medium">Uptime 99.9%</span>
        </div>
      </div>
      <div className="absolute -left-3 bottom-32 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-lg">
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-slate-700 font-medium">1,234 msgs today</span>
        </div>
      </div>
    </div>
  );
