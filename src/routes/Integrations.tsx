...
export default function Integrations() {
  ...
  return (
    <div className="px-4 sm:px-8 py-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search integrations..."
            className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all" />
        </div>
        <div className="flex gap-2">
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setCategory(cat.id)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                category === cat.id ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/20' : 'bg-white/[0.03] text-slate-500 border border-white/5 hover:text-white'
              }`}>
              {cat.label}
            </button>
          ))}
      ...
      {category === 'all' && !search && (
        <div className="mb-8">
          <h2 className="text-sm font-medium text-amber-400 mb-3 flex items-center gap-2">Recommended for you</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {integrations.filter((i) => i.popular && i.status === 'available').slice(0, 3).map((i) => (
      ...
      <div>
        <h2 className="text-sm font-medium text-slate-500 mb-3">
          ...
          <span className="ml-2 text-slate-600">({filtered.length})</span>
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((i) => (<IntegrationCard key={i.id} integration={i} onConnect={() => {}} />))}
        </div>
      </div>
    </div>
  );
