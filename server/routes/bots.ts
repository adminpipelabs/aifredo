...
router.post('/', (req, res) => {
  ...
  const { name, persona, model, channel, system_prompt, status } = req.body;
  if (!name?.trim()) return res.status(400).json({ message: 'Bot name is required' });

  const db = getDb();
  const id = crypto.randomUUID();

  // Allow 'online' status when activating, default to 'offline'
  const botStatus = status === 'online' ? 'online' : 'offline';
...
// ─── Public bot info (no auth — for shareable chat page) ─────
router.get('/public/:id', (req, res) => {
  const db = getDb();
  const bot = db.prepare('SELECT id, name, persona, model, channel, system_prompt, status FROM bots WHERE id = ?').get(req.params.id) as any;
  if (!bot) return res.status(404).json({ message: 'Bot not found' });

  // Only expose safe fields — never expose API keys, tokens, or user_id
  res.json({
    bot: {
      id: bot.id,
      name: bot.name,
